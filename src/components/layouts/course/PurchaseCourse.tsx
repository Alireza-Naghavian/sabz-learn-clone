...
type PurchseType = { isFree: boolean, price:number,  discount:number,_id:string,shortName:string}
function PurchaseCourse({ isFree,price,discount,_id,shortName }:PurchseType ) {
    ...
    const registerHandler = async()=>{
        try {
          if(!isUserLoading && data==undefined || data?.user == undefined) return showAlert("error","لطفا ثبت نام کنید/وارد شوید")
            const result = await registerCourse({price,_id,shortName}).unwrap();
    ...
    return (
        <>
          {/* countDown ? */}
          {discount !==0 && <CountDowntimer date={new Date("dec 25, 2024 16:37:52")} percent={discount} />}
          <div
            ...
            <PrimaryBtn
              ...
              onClick={()=>{
                if(isFree){
                  ...
                  router.push(`/courses/course/register/${_id}`,{scroll:true})
                }
              }}
            >
            ...
            <div className="flex items-end gap-x-2.5">
              {/* offer ? */}
                ...
                <span className="text-slate-500 dark:text-white/70 text-xl line-through">
                 {
                 discount !==0 ? price.toLocaleString("fa-IR")
                 :
                 isFree && `${price.toLocaleString("fa-IR")}`}
                </span>
          
              {/* price*/}
              <span className={`font-DanaBold text-2xl flex items-center gap-x-1 ${isFree && "text-baseColor"}`}>
                <span>{
                 discount !==0 ?( price - (Number(price * discount) /100)).toLocaleString("fa-IR")
                :
                !isFree ? price.toLocaleString("fa-IR") : "رایگان"}</span>
               {!isFree ? <TomanIcon className="size-6" /> : 
